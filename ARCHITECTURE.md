...
* **ClaimsModule.Domain:** Zero external package dependencies. Houses aggregates, entities, domain events, enums, and the `Money` and `ClaimNumber` value objects.
...
To safeguard accounting integrity, reserve balances have no persisted `CurrentAmount` column. Balance modification follows an append-only architecture:
...
* **Tenant Isolation:** Enforced transparently through global EF query filters using the assessment's fixed seeded organisation. Tenant-owned and reference entities carry `OrganisationId`; the sequence table is tenant-keyed.
...
| **Atomic Counters over UUID Generators** | Uses a SQL Server `MERGE ... WITH (HOLDLOCK)` counter inside the same transaction as claim creation. **Benefit:** prevents duplicate numbers and rolls counter allocation back if claim persistence fails. |
...
The production build embeds the Angular browser output in the API artifact. This keeps the vertical slice on one origin, avoids permissive production CORS, and makes one atomic App Service deployment possible. `infra/main.bicep` is the source of truth for cloud resources.